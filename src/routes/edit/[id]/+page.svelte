...
<div class="container">
  <div class="topbar">
    <div>
      <h1 style="margin:0;">Produkt bearbeiten</h1>
      <p style="margin:6px 0 0; color:#6b7280;">Passe Angaben zur Garantie an.</p>
    </div>
  ...
  <div class="card">
    <form method="POST" class="form">
      <div class="field">
        ...
        <input id="name" name="name" value={form?.values?.name ?? p.name} />
        {#if form?.errors?.name}<div class="error">{form.errors.name}</div>{/if}
      </div>

      <div class="field">
        <label for="retailer">Händler</label>
        <input id="retailer" name="retailer" value={form?.values?.retailer ?? p.retailer} />
      ...
      <div class="field">
        ...
        <input
          ...
          value={form?.values?.receiptUrl ?? (p.receiptUrl ?? "")}
        />
